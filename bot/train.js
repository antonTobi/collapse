...
const { Network, NT, SIZE } = require('./ntuple.js');

function parseArgs(argv) {
    // maxMoves matters: a value function trained purely to survive can find
    // near-endless lines. The game does terminate (every 6 is permanent, 5s
    // only clear by making a 6, and so on down, bounding a game at ~25^5 moves)
    // but that bound is hours of compute, which silently stalls training.
    const a = { episodes: 20000, alpha: 0.1, out: path.join(__dirname, 'weights/td.bin'), resume: null, seedBase: 100000, report: 1000, decay: 1, maxMoves: 5000 };
...
function save(file, w) {
    fs.mkdirSync(path.dirname(file), { recursive: true });
    fs.writeFileSync(file, Buffer.from(w.buffer, w.byteOffset, w.byteLength));
}
...
function main() {
    ...
    const net = new Network(args.resume ? load(args.resume) : undefined, args.sym);
    ...
    for (let ep = 0; ep < args.episodes; ep++) {
        ...
        while (cur && game.moves.length < args.maxMoves) {
            ...
            net.update(cells, alpha * (target - net.value(cells)));
            cur = next;
        }
        ...
        if ((ep + 1) % args.report === 0) {
            ...
            console.log(`ep ${ep + 1}  mean(last ${window.length}) ${mean.toFixed(0)}  alpha ${alpha.toFixed(4)}  capped ${capped}  ${((Date.now() - t0) / 1000).toFixed(0)}s`);
            save(args.out, net.w);
        }
        alpha *= args.decay;
    }
    save(args.out, net.w);
    console.log('saved ' + args.out + ` (${NT} tuples x ${SIZE} = ${net.w.length} weights)`);
}
